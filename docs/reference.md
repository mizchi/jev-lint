...
| `eval` | `suites[]` with `ok`, `score` (each rule scored: `tp`/`fp`/`fn`, `precision`, `recall`, `cleanTop`, `fnMargin`, `fpMargin`, `fnSpread`, `fpSpread`, `blind`, `unstable`, `inconclusiveReason`), `diff`, `changedDrafts`, `recorded`, `passes`, `unanswered` (missing answers counted per pass); `failed`. With `--dry-run`, each suite's `plan` |
...
99 rules under `rules/<lang>/<id>/`, each with its fixtures beside it,
...
Of the 99, **79 reach precision 1.00 and recall 1.00 at their shipped cutoffs
...
one to thirty-seven labelled defects — 707 in all, against 113 before the
improvement round of 2026-09-20 and 644 before the boundary round of
2026-09-22. The first added hard cases drawn from real code; the second aimed
them at each rule's cutoff rather than a comfortable distance from it, which
is why the count above fell from 87 of 98 as it rose. `jev-lint eval` now
refuses a suite whose fixtures are all far enough from the boundary that they
could not see the rule drift — see [RULES.md](../RULES.md) for the current
per-rule numbers, which this page does not repeat. The two `module-name-describes-contents`
...
reports two distances beside `cleanTop`, both from the SHIPPED cutoff and
both normalised by the rule's own scale (`scaleOf`: 0–1 for a `noul`, the
rubric's own top for a `score`, so a margin of 1.02 on a five-level rubric
reads the same as 0.26 on a `noul`) — normalised, because otherwise the two
kinds are not comparable and a markdown rule's raw margins would look wide
next to a noul's for no reason but the scale:

- **FN-margin** — the quietest labelled defect's mean, minus the cutoff.
  Small means a defect sits near the boundary; a rule that starts missing
  things fails the eval before it fails silently on real code.
- **FP-margin** — the cutoff, minus the loudest labelled clean's mean.
  Small means a clean sits near the boundary; a rule that starts
  over-flagging fails the eval the same way.

Only an *explicit* `bad`/`clean` entry in `expect.yml` sets a margin — never
`$default: clean`, which resolves every subject nobody wrote a line for.
`cleanTop` and precision/recall still resolve through `$default`, as they
always have; a margin does not, because it claims someone actually looked
at this specific location and it is the quietest defect or the loudest
clean found, and a line nobody chose is not that evidence. A suite with no
...
- **The cutoffs are fitted to small evals.** 497 labelled fixtures, 707
  labelled defects and 1,135 labelled cleans across 99 rules, one to
  thirty-seven defects per rule. Expect to refit; see
...
- **Fields may not exist.** MoonBit's grammar declares none, so
  ...
  capture a name.
- **Rules for it may ship.** `rules/moonbit/` is in the package: twenty
  rules -- every rule of this tool that a MoonBit file can carry -- each
