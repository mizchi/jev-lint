...
| `.mbt` | `moonbit/` — 15 | MoonBit のファイルが持ちうるルールは全部: 命名、保証、コメント、失敗契約、メッセージ、テスト。MoonBit は ast-grep 組み込みの文法ではないので、[parser を宣言](docs/reference.md#a-language-ast-grep-does-not-have-built-in)すれば動く。宣言が無ければ skip され、その旨が出る |
