...
これにインストールは要らない。Node 24+。`check` は同梱ルール全部 (`rules/<language>/<id>/` の下の 98 個) を読み込み、どれが走るかは渡したファイルが決める。`.ts` には TypeScript のルールが、`.md` には Markdown のルールが当たり、ファイルの無い言語は実行の最後に idle として列挙される。選ぶものはない。
