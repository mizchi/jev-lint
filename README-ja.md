...
| `.mbt` | `moonbit/` — 3 | 命名・コメント・テストのルールの移植。MoonBit は ast-grep 組み込みの文法ではないので、[parser を宣言](docs/reference.md#a-language-ast-grep-does-not-have-built-in)すれば動く。宣言が無ければ skip され、その旨が出る |
