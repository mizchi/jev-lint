...
```
experiments/rule-candidates/<lang>/<rule-id>/
  rule.yml             the candidate rule, one language (typescript admits the ECMAScript four)
  fixtures/            the labelled cases
  expect.yml           hand-written, paths relative to the rule dir, format below
  baseline.json        written by `jev-lint eval <dir> --accept`
experiments/reports/<family>/REPORT.md   the report, format below
```

A second language is a second directory with the same id under
`experiments/rule-candidates/<other-lang>/`.
...
fixtures/a.ts:
  ...
  - { line: 30, label: clean, window: 0, reason: "hard clean: ..." }
```

Paths are relative to the rule directory. `window: 0` for subjects that sit one
...
4. `gaps <dir>/fixtures -R <dir>/rule.yml --no-config --cache none` ->
...
A candidate that ships is `git mv experiments/rule-candidates/<lang>/<id> rules/<lang>/<id>`;
