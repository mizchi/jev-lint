...
| `.mbt` | `moonbit/` — 10 | ports of the naming, guarantee, comment and test rules. MoonBit is not a grammar ast-grep has built in: [declare the parser](docs/reference.md#a-language-ast-grep-does-not-have-built-in) and these run; without it they are skipped and the run says so |
