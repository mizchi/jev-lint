...
| `.mbt` | `moonbit/` — 15 | every rule of this tool that a MoonBit file can carry: the naming rules, the guarantees, the comments, the failure contract, the messages, the tests. MoonBit is not a grammar ast-grep has built in: [declare the parser](docs/reference.md#a-language-ast-grep-does-not-have-built-in) and these run; without it they are skipped and the run says so |
